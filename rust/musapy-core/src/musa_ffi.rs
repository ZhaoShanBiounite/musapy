...
mod real {
    ...
    // --- Memory（ADR L3-9, stream-ordered alloc/free）---
    pub fn musaMallocAsync(
        devPtr: *mut *mut c_void,
        size: usize,
        stream: musaStream_t,
    ) -> musaError_t;
    pub fn musaFreeAsync(
        devPtr: *mut c_void,
        stream: musaStream_t,
    ) -> musaError_t;
...
// --- Memory（mock stub）---
    ...
    pub unsafe fn musaMallocAsync(
        ...
        // mock：用 std::alloc 分配真实内存，让指针非空且可读写
    ...
    pub unsafe fn musaFreeAsync(
        ...
        if let Some(idx) = allocs.iter().position(|&p| p == devPtr as usize) {
            ...
            // mock：不真正 dealloc（不知道 size），靠进程退出回收
        }
        MUSA_SUCCESS
    }
...
/// 探测设备是否支持内存池（stream-ordered alloc/free，ADR L3-11）。
///
/// 返回 `true` 表示支持 `musaMallocAsync`/`musaFreeAsync`，可走全量 stream-ordered 方案。
/// 返回 `false` 表示需要走 deferred-free fallback（Phase 3 第三批实现）。
///
