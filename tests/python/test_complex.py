...
class TestComplexArray:
    ...
    def test_literal_infers_c128(self):
        ...
        assert a.dtype == ms.complex128
        assert a.tolist() == [1 + 2j, 3 - 4j]

    def test_explicit_c64(self):
        a = ms.array([1.0, 2.0, 3.0], dtype=ms.complex64)
        assert a.dtype == ms.complex64
    ...
    def test_explicit_dtype_overrides_inference(self):
        a = ms.array([1 + 2j, 3 + 4j], dtype=ms.complex64)
        assert a.dtype == ms.complex64
...
class TestComplexElementwise:
    ...
    def test_binary_ops(self, dtype):
        ...
        for name, got, exp in [
            ...
            tol = 1e-5 if dtype == ms.complex64 else 1e-10
    ...
    def test_unary_neg_abs(self, dtype):
        ...
        assert np.allclose(
            np.array(ms.neg(x).tolist()), -xn, atol=1e-5 if dtype == ms.complex64 else 1e-10
        )
        # abs 输出 real（NumPy 语义）
        ab = ms.abs(x)
        real_dtype = ms.float32 if dtype == ms.complex64 else ms.float64
        assert ab.dtype == real_dtype
        assert np.allclose(
            np.array(ab.tolist()), np.abs(xn), atol=1e-5 if dtype == ms.complex64 else 1e-10
        )
    ...
    def test_ordering_rejected(self, fn):
        x = ms.array([1 + 2j, 3 + 4j], dtype=ms.complex128)
        y = ms.array([1 + 1j, 2 + 2j], dtype=ms.complex128)
...
class TestComplexPromotion:
    ...
    def test_f64_add_c64(self):
        ...
        b = ms.array([1 + 1j, 2 - 2j], dtype=ms.complex64)
        r = ms.add(a, b)
        assert r.dtype == ms.complex128
    ...
    def test_f32_add_c64(self):
        ...
        b = ms.array([1 + 1j, 2 - 2j], dtype=ms.complex64)
        r = ms.add(a, b)
        assert r.dtype == ms.complex64

    def test_f32_add_c128(self):
        a = ms.array([1.0, 2.0], dtype=ms.float32)
        b = ms.array([1 + 1j, 2 - 2j], dtype=ms.complex128)
        r = ms.add(a, b)
        assert r.dtype == ms.complex128
    ...
    def test_c64_add_c64_narrow(self):
        a = ms.array([1 + 1j], dtype=ms.complex64)
        b = ms.array([2 + 2j], dtype=ms.complex64)
        assert ms.add(a, b).dtype == ms.complex64
