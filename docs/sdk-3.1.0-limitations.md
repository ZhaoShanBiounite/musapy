...
> **日期**:2026-08-09（合并 sdk-3.1.0-musax-coverage.md：§0 符号覆盖清单；§1.8-1.11 为 2026-08-07/08 benchmark 段级探针新增，§1.12-1.16 为 mcc 编译器限制）
...
## 0. 符号覆盖清单（v0.3 Phase 1 P1.1 核对结论，已并入）

**结论**:v0.3 计划(Phase 2–6)所需符号 **83/83 在位**,头文件与 .so 一致（`tools/check_musax_symbols.sh`
96/96 PASS）,v0.3 风险登记表「muSOLVER 运行期符号与头文件不一致」一栏维持**低**概率评级。

### 0.1 库文件形态

| 库 | 头文件 | 动态库 |
|---|---|---|
| muBLAS | `mublas.h` | `libmublas.so` |
| muSOLVER | `musolver.h` | `libmusolver.so` |
| muRAND | `murand.h` | `libmurand.so` |
| muFFT | `mufft.h` | `libmufft.so` + `libmtfft-device-*.so`（按 arch 变体） |
| muSPARSE | `musparse.h` | `libmusparse.so` |

### 0.2 muBLAS（Phase 2:matmul/dot）
...
| `mublasSgemmStridedBatched/DgemmStridedBatched` | ✅ | batch matmul 候选(v0.4 评估) |

### 0.3 muSOLVER（Phase 2 solve / Phase 3 lu/qr/svd）
...
### 0.6 muSPARSE（Phase 6:sparse 套件）
